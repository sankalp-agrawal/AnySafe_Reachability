...
from real_envs.env_utils import normalize_eef_and_gripper
from test_loader import SplitTrajectoryDataset
from torch.utils.data import DataLoader
from tqdm import tqdm

dino = torch.hub.load("facebookresearch/dinov2", "dinov2_vits14_reg")
saferl_dir = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__), "../Lipschitz_Continuous_Reachability_Learning"
    )
)
...
def fill_eps_from_pkl_files(cache, cache_eval):
    ...
    pkl_files = [
        os.path.join(demo_path, f) for f in os.listdir(demo_path) if f.endswith(".pkl")
    ]
...
if __name__ == "__main__":
    ...
    expert_data_imagine = SplitTrajectoryDataset(
        hdf5_file, bl, split="train", num_test=0
    )
    ...
    while True:
        ...
        while (data["failure"][[0], :9] == 0).all() or (
            data["failure"][[0], 6:] > 0
        ).all():
            data = next(expert_loader_imagine)
        ...
        im1s = (
            (data["agentview_image"][[0], :H].squeeze().to(device) / 255.0)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        pred_brts = []
        for i in range(bl - H):
            latent = transition.forward_features(inputs1, states, acs)

            pred1, pred_state, pred_fail = (
                transition.front_head(latent),
                transition.state_pred(latent),
                transition.failure_pred(latent),
            )
            ...
            # getting next inputs
            acs = torch.cat(
                [acs[[0], 1:], all_acs[0, H + i].unsqueeze(0).unsqueeze(0)], dim=1
            )
        ...
        gt_im1 = (
            (data["agentview_image"][[0], :bl].squeeze().to(device) / 255.0)
            .detach()
            .cpu()
            .numpy()
        )

        gt_imgs = np.concatenate([gt_im1], axis=-3)
        pred_imgs = np.concatenate([im1s], axis=-3)
        ...
        im2s = (
            (data["robot0_eye_in_hand_image"][[0], :H].squeeze().to(device) / 255.0)
            .detach()
            .cpu()
            .numpy()
        )
        ...
        for i in range(bl - H):
            ...
            pred1, pred2, pred_state, pred_fail = (
                transition.front_head(latent),
                transition.wrist_head(latent),
                transition.state_pred(latent),
                transition.failure_pred(latent),
            )
            ...
            inputs1 = torch.cat(
                [inputs1[[0], 1:], all_in1s[H + i].unsqueeze(0).unsqueeze(0)], dim=1
            )
            inputs2 = torch.cat(
                [inputs2[[0], 1:], all_in2s[H + i].unsqueeze(0).unsqueeze(0)], dim=1
            )
            states = torch.cat(
                [states[[0], 1:], all_states[0, H + i].unsqueeze(0).unsqueeze(0)], dim=1
            )
        ...
        gt_im2 = (
            (data["robot0_eye_in_hand_image"][[0], :bl].squeeze().to(device) / 255.0)
            .detach()
            .cpu()
            .numpy()
        )
