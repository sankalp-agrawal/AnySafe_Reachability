...
class Logger:
    def __init__(self, logdir, step):
        ...
        # Initialize WandB
        wandb.init(
            project="World Model Training", config={"logdir": str(logdir)}, name=name
        )
    ...
    for i, demo in tqdm(
        ...
        for t in range(len(traj["obs"][pixel_keys[0]])):
            ...
            if "margin" in config.grad_heads:
                ...
                #     dtype=np.float32,
                # )
                transition["failure"] = traj["failures"][t]
                transition["constraint_state_gt"] = traj["constraints"][
                    "constraint_state_gt"
                ][t]
                transition["constraint_img"] = traj["constraints"]["constraint_img"][t]
