...
class SplitModule:
    def __init__(self, save_path, sz_feature=512, sz_embed=64):
        self.splitnet = SplitNet(sz_feature=sz_feature, sz_embed=sz_embed)
        self.splitnet = self.splitnet.cuda()
    ...
    def generate_dataset(
        self, dataset, index, index_target=None, target=None, index_target_new=None
    ):
    ...
    def calc_old_new(
        ...
        if step > 0:
            print("Normalizing...")
            preds_cs = (
                (preds_cs - preds_cs.min()) / (preds_cs.max() - preds_cs.min())
            ) * 2.0 - 1.0
    ...
    def calc_GMM_cross(
        ...
        total_loss_org = (total_loss - total_loss.min()) / (
            total_loss.max() - total_loss.min()
        )
    ...
    def split_old_and_new(
        ...
        param_groups = [
            ...
            {"params": self.splitnet.parameters()},
        ]
        ...
        dl_tr_o = torch.utils.data.DataLoader(
            ev_dataset_o,
            batch_size=batch_size,
            shuffle=True,
            num_workers=num_workers,
        ...
        for epoch_main in range(main_epoch):
            for epoch_sub in range(sub_epoch):
                ...
                print(
                    "Fine. Split result Epoch: {}/{} Loss: {}".format(
                        epoch_main, epoch_sub, np.mean(total_loss_arr)
                    )
                )

            m_, y_true, _ = self.predict_batchwise_split(
                main_model_, self.splitnet, dl_ev
            )
