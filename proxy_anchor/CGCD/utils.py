    ...
    normalize_un = transforms.Compose(
        [
            transforms.Normalize(
                mean=[-0.4914 / 0.2471, -0.4824 / 0.2435, -0.4413 / 0.2616],
...
def show_OnN(m, y, v, nb_classes, pth_result, thres=0.0, is_hist=False, iter=0):
    """
    Graph histogram of the old and new classes based on the given threshold.
    """
    # o = old, n = new
    ...
    print(
        "Init. Split result(0.)\t oo: {}\t on: {}\t no: {}\t nn: {}".format(
            oo_i, on_i, no_i, nn_i
        )
    )
